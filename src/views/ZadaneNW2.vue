<template>
  <div class="page">
    ...
    <div class="pagebody">
      ...
      <table class="table">
        <tr>
          <td>Tryb pracy NW2</td>
          <td>
            <select v-model="form.s16421">
      ...
      <table class="table">
        <tr v-for="item in pressureFields" :key="item.point">
          <td>{{ item.label }}</td>
          <td>
            <input v-model="form[item.point]" type="number" step="1" />
            Pa
      ...
      <table class="table">
        <tr>
          <td>Tryb zadawania temperatury nawiewu NW2</td>
          <td>
            <select v-model="form.s16449">
              <option value="0">Auto</option>
              <option value="1">Man</option>
            </select>
            <button @click="send('s16449')">Zapisz</button>
          </td>
        </tr>

        <tr>
          <td>Zadana temperatura nawiewu N2 (dla trybu man)</td>
          <td>
            <input v-model="form.t16450" type="number" step="0.1" />
            °C
            <button @click="send('t16450')">Zapisz</button>
          </td>
        </tr>

        <tr>
          <td>Zadana wilgotność bezwzględna N2</td>
          <td>
            <input v-model="form.t16458" type="number" step="0.1" />
            g/kg
            <button @click="send('t16458')">Zapisz</button>
          </td>
        </tr>
      </table>

      <p class="sectiontitle">Zadane pomieszczeń</p>
      <table class="table">
        <tr>
          <td>Zadane wysterowanie regulatora ciśn. pom. 022a</td>
          <td>
            <input v-model="form.t16434" type="number" step="0.1" />
            %
            <button @click="send('t16434')">Zapisz</button>
          </td>
        </tr>
      ...
      <table class="table">
        <tr v-for="item in scheduleFields" :key="item.point">
          <td>{{ item.label }}</td>
          <td>
